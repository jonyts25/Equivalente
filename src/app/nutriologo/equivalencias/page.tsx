...
export default async function NutriologoEquivalenciasPage() {
  ...
  let groupList: EquivalenceGroupRow[] = [];
  if (nutritionist) {
    const { data: groups } = await supabase
      .from("equivalence_groups")
      .select(
        "id, name, category, notes, equivalence_items(portion_label, grams, units, notes, food_items(name))"
      )
      .eq("nutritionist_id", nutritionist.id)
      .is("patient_id", null)
      .eq("active", true)
      .order("name");
    groupList = (groups ?? []) as unknown as EquivalenceGroupRow[];
  }

  const hasEquivalences = groupList.some((g) => (g.equivalence_items?.length ?? 0) > 0);

  return (
    <AppShell title="Equivalencias" nav={NUTRIOLOGO_NAV} currentPath="/nutriologo/equivalencias">
      <p className="text-sm text-slate-600">
        Tabla general de equivalencias de tu consultorio. Se usa al generar opciones para
        cualquier paciente.
      </p>
      ...
      <Card>
        ...
        <CardContent className="space-y-3">
          {groupList.length === 0 && (
            <p className="text-sm text-slate-600">
              Aún no hay grupos. Usa el formulario abajo para agregar uno.
            </p>
          )}
          {groupList.map((g) => {
        ...
        <Card>
          ...
          <CardContent>
            <p className="mb-3 text-xs text-slate-500">
              Crea el grupo aquí; los ítems se agregan desde el catálogo o con soporte clínico
              posterior.
            </p>
            <EquivalenceForm />
          </CardContent>
